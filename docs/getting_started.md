...
- A library `ppx_deriving_popper` — for deriving functions to *sample* and
*compare* test data for custom data-types.
...
- `discarded` — the proposition is neither `true` nor `false`, the test is discarded.
...
confirms that a certain *property* holds for a large number of sampled data
points.  The technique was popularized by the Haskell library
...
In the test, `xs` is a *sample* of type *list of integers*. The test returns a
...
The function `is_true` is used to construct a proposition stating that the given
...
drawn sample for which the condition `is_sorted` did not hold, we may add add
...
may derive it using `[@@deriving show]`. We'll soon look at how to also derive
...
unit-test is only run once. How does `check` know how to distinguish between the
...
data for drawing (pseudo-random) samples. A value of type `a Sample.t` is really
a *parser* that reads input from a stream of numbers (`int32` values) and
...
- `suite` for naming and grouping tests.
...
!!! note
    ...
    Deriving popper for a type called `t`, generates functions called
    `sample` and `comparator`, rather than `t_sample` and `t_comparator`.
    This is to mirror the deriving behavior for `show` and `ord`.
...
For the complete list, see the module [Popper.Config](https://jobjo.github.io/popper/api/popper/Popper/Config/index.html).

First, let's see what happens when we run a test configured with `verbose`.
Here's a modified version of the `test_or` function from above:
...
!!! note
    ...
    `Config.max_size`. This, of course, limits the test coverage.
